      ...
      <div v-for="item in orderResults" :key="item.id"
        ...
        <div v-if="item.address" style="width:880px; border: 1px transparent solid;; float:left;  line-height: 30px;padding-left: 10px ;">
         ...
         <span style="display: inline-block;margin-left: 100px">收货人手机：{{item.address.tel}}</span>
          ...
          <div>
            <el-button  size="mini" slot="reference"  round @click="changeGoodState(item.id)">评价</el-button>
          <el-button size="mini" slot="reference" @click="editComplaint(item.id)" round>投诉</el-button>
          </div>
         </span>
        </div>
      </div>
    </div>
    <EditScore  ref="edit"></EditScore>
    <EditComplaint  ref="edit1"></EditComplaint>
  </div>
</template>
...
<script>
  ...
  export default {
    components:{EditScore,EditComplaint},
    name: "",
    data() {
      return {
        state:'2',
    ...
    methods:{
      editComplaint(orderId){
        this.$refs.edit1.dialogFormVisible = true;
        this.$refs.edit1.id=orderId
        this.$refs.edit1.state=this.state
      },
      changeGoodState(orderId){
        this.$refs.edit.dialogFormVisible = true;
        this.$refs.edit.id=orderId
        this.$refs.edit.state=this.state
      }
    },
    created: function () {
      var token;
