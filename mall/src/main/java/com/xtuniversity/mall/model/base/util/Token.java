...
public class Token {
	...
	private final static AtomicInteger[] SEN = { new AtomicInteger((int) (System.nanoTime())), new AtomicInteger((int) (System.nanoTime())), new AtomicInteger((int) (System.nanoTime())), new AtomicInteger((int) (System.nanoTime())), new AtomicInteger((int) (System.nanoTime())) };
	...
	public static Token init(String ipAddr, long userId) {
	...
	public static boolean checkToken(String value) {
		...
		for (int i = DATA_LENGTH, j = 0; i < chs.length; i++, j++) {
			if (chs[i] != hash[j]) {
				log.warn("value hash is invalid, value hash: [" + new String(chs, DATA_LENGTH, TOKEN_LENGTH - DATA_LENGTH) + "], calculate hash: [" + new String(hash) + "], token: [" + value + "]");
				return false;
			}
		}
		return true;
	...
	public static boolean checkUserToken(String tokenString, long userId) {
